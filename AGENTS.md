...
- Tests: `uv run pytest tests/ -q -n auto` (parallel, ~1min, pytest-xdist in dev group). Teardown-hang hardening is in `tests/conftest.py` (per-test: close DB connections + cancel leftover asyncio tasks; sessionfinish prints leftover non-daemon threads; `faulthandler_timeout=120` auto-dumps stacks if a single test gets stuck). If a run still hangs at exit, the last stderr lines name the culprit thread; a stuck worker dumps stacks after 120s (dump only — kill the process manually). Legacy serial run: `timeout 300 uv run pytest tests/ -q`. On Linux a handful of tests fail or hang mid-run (platform-specific, Windows-first — NOT a regression): PowerShell cases in `test_slack_clone_01_retro_fixes.py`, `rmtree` cases in `test_husk_and_evidence_gate_fixes.py`, `test_hire_market_last_search.py` mock-await, `test_workspace_shared_tracking.py`/`test_worktree_p0_lifecycle.py` worktree-reconcile.
- Typecheck: `mypy` is a declared dev dependency (`[project.optional-dependencies].dev` + `[dependency-groups].dev`), installed by `uv sync --extra dev`. Run `uv run mypy src/hiveweave/ --ignore-missing-imports` (from `apps/hiveweave-py`). There are ~56 pre-existing type errors — not a regression.
