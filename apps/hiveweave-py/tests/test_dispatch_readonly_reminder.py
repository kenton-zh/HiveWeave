"""dispatch_task hard gates: coordinator assignee block + direct-report span.

行为契约（2026-07 升级：软提醒 → 硬拒绝）:
- 派发给 permission_type='coordinator'：硬拒绝，不调用 DispatchService
- 派发给非直属下属：硬拒绝跨级
- 派发给直属 executor：成功
...
class TestDispatchHardGates:
    ...
    async def test_dispatch_to_coordinator_hard_blocked(self):
        """派发给 coordinator：硬拒绝，不调用底层 dispatch。"""
        ...
        assert "拒绝派活" in text or "coordinator" in text.lower()
