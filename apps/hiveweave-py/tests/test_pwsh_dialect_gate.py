"""P0-2 (TEST_DSH_33): bash→pwsh 方言 gate + pwsh 声明式工具。

TEST_DSH_33 实测 126 次方言失败（占失败步 41.9%，6 Agent 全中、跨 422 分钟
未收敛）：受限沙箱 shell = pwsh，而翻译表只覆盖明确形态。gate 的契约：
- 翻译表能 1:1 对上的 → 翻译后放行（不再报「不是内部或外部命令」）
- 对不上的 → 出**带等价写法**的可操作错误（不得猜译——grep -rn 曾被
  译成 `Select-String -Pattern -rn`，把方言失败变成静默错译）
...
@pytest.mark.parametrize(
    "command",
    [
        "head -n 800 docs/design.md",   # TEST_DSH_33 直调复现用例（曾原样透传）
        "head -80 docs/design.md",
        "tail -n 20 out.log",
        "tail -20 out.log",
        "ls -lh src",                   # 曾透传后报「不是内部或外部命令」
        "ls -la src",
        # 曾被猜译成 `Select-String -Pattern -rn "judge" src/`（静默错译）
        'grep -rn "judge" src/',
        "wc -l x.ts",
        "mkdir -p build/out",
        "cat app.log | grep warning",   # 管道尾 grep → Select-String
        "Get-Content x -TotalCount 80",
        "python script.py",
        "echo hello world",              # 裸 echo 无 unix flag → pwsh 兼容
        "git status",
    ],
)
def test_gate_passes_translatable_or_native(gate_on, command):
