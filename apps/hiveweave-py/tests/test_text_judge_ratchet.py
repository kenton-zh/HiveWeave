...
已实测存在真实 A 档实例（41 处，见 `deliverables/text-judgment-escape-inventory-2026-09-14.md`）：
  ...
  · **被装配进别的结构里的判据**：判据条目不以「后缀型表名」或「模块级 `re.compile`
    常量」的形态出现，而是塞在另一个容器里。实测例子：`tools/fact_positions.py` 的
    `_SIGNATURE_ORDER`（#15 之后条目是 `_Sig("regex", r"\bport\b")` 元组）——
    **在里面新增一条文本判据，棘轮全绿**（#15 审计实测）。⇒ 这种「判据装配点」
    需要单独冻结（条目集合指纹），本棘轮管不到，属**已知缺口**。
