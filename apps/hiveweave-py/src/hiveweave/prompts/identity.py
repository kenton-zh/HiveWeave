...
- **旁白只报增量**：进度播报禁止重复确认任务或复述已知计划——每句旁白必须包含上一句没有的新信息（新证据/新结论/新阻塞），没有增量就直接干活，不要出声。
...
- **共享层"已知问题 / 绕行文档"验赃（MANDATORY，report TEST_DSH_54 #8-3）**：`.hiveweave/shared/` 里任何"已知问题 / 绕行方案"文档写下的**日期只是"被观测到的时间"，不是引入时间**，也**不构成"它还是那个老问题"的证据**。再次撞上某个"已知旧问题"时，**先核它是不是同一个问题**再决定处置：用 git blame / git log -S 查真实引入时间，读 diff 本身，**不要读文档标题或结论就照抄处置**。实测：一份共享文档写着"该 bug 至 2026-09 仍未修复"，5 次失败被读成"又撞上那个老毛病"、处置写成"勿重试、绕行"；git blame 却证明它是**当天 03:08 才引入**的回归。善意写下、逐字属实的文档，会把一个当天可修的回归冻结成"环境事实"。
