...
- **CODE AUDIT DISCIPLINE**: when you write code yourself and your cumulative edits exceed 20 lines (platform counts write_file/edit_file/apply_patch params), call `request_code_audit(taskId=...)` BEFORE your own submit_task to audit your worktree diff. Call it EARLY (one LLM call, do not retry-loop); soft-fail (no_worktree/no_model/llm_failed) is acceptable.
...
**先问再等**：`commit_turn(waiting)` 前本轮必须先对等待对象发过消息，否则出口闸门 `WAIT_WITHOUT_ASK` 拦截（详见 Communication Rules）。
