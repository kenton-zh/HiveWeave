...
# P1/P1b split — re-export so `from hiveweave.agents.agent import …` keeps working
from hiveweave.agents.constants import (  # noqa: F401
    SAFETY_TIMEOUT_MS,
    EMPTY_RETRY_DELAYS,
    MAX_EMPTY_RETRIES,
    TRIGGER_DELAY_MS,
    SELF_RETRIGGER_DELAY_MS,
...
from hiveweave.agents.types import (  # noqa: F401
    AgentState,
    StatusCallback,
    StreamEventCallback,
)
...
from hiveweave.agents.helpers.tools_def import (  # noqa: F401
    _build_tool_definitions,
    _short_hash,
)
from hiveweave.agents import streaming as _agent_streaming
...
async def broadcast_agent_health(
    ...
    """Module-level health broadcast by agent_id (misc_tools / external callers).

    Looks up the live Agent in agent_manager and delegates to streaming helper.
    No-op if the agent is not running — never raises to callers.
    """
    try:
...
class Agent:
    ...
    async def _inbox_watcher_loop(self) -> None:
        ...
        使用指数退避重试 [5s, 15s, 45s]。
        """
        return await _agent_watcher.inbox_watcher_loop(self)
    ...
    def _ensure_watcher_alive(self) -> None:
        ...
        """
        return _agent_watcher.ensure_watcher_alive(self)
    ...
    async def _handle_completion(
        ...
        5. 自检 re-trigger
        """
        return await _agent_completion.handle_completion(self, result, message, opts)
    ...
    async def _handle_empty_response(
        ...
        """
        return await _agent_recovery.handle_empty_response(self, result, current_messages)
    ...
    async def _park_after_stream_timeouts(
        ...
        """
        return await _agent_recovery.park_after_stream_timeouts(self, inbox_ids=inbox_ids, error_msg=error_msg)
    ...
    async def _escalate_turn_interruption(self, *, reason: str) -> None:
        ...
        """
        return await _agent_recovery.escalate_turn_interruption(self, reason=reason)
    ...
    async def _handle_cancel(self) -> None:
        ...
        """
        return await _agent_recovery.handle_cancel(self)
    ...
    async def _escalate_unreplied(self, unreplied_msgs: list[dict]) -> None:
        ...
        """
        return await _agent_recovery.escalate_unreplied(self, unreplied_msgs)

    async def _escalate_trigger_fuse(self, pending: list[dict]) -> None:
        """Fuse tripped: notify parent, keep actionable inbox unread."""
        return await _agent_recovery.escalate_trigger_fuse(self, pending)
    ...
    def _broadcast_status(self, status: str, extra: dict | None = None) -> None:
        ...
        return _agent_streaming.broadcast_status(self, status, extra)

    def _broadcast_stream_event(self, event: dict) -> None:
        """广播流事件（通过回调，不直接依赖 WebSocket）。"""
        return _agent_streaming.broadcast_stream_event(self, event)
