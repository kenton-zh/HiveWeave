...
APPROVAL_TIMEOUT_HINT = (
    ...
    "（超时 %ds，未批准也未拒绝）。平台无法观测到审核人是否在线，"
    "也无从判断这是否属于无人值守运行 —— 本分支不对「运行方式」"
    "或「谁失职」做任何断言。"
...
# 无人值守模式的**直接**拒绝文案（2026-09-13 TEST_DSH_55 报告 §3 第 6 条）。
# 这条路径**没有发起过审批请求、也没有等待**（`is_unattended_mode` 为真时
# 直接走替代方案）—— 过去复用 APPROVAL_TIMEOUT_HINT 会让它声称「审批请求
# 超时（超时 120s，未批准也未拒绝）」，而事实是**从未请求过**。文案与实际
# 不符 = 给模型错误事实（模型会据此误判"审核人失职"或"平台在等我"）。
# 实测：本项目的 `unattended_mode` 并未登记（5 个 key 里没有它），而报告
# 那条超时文案却断言「无人值守是既定方式」—— 断言与事实相反。
UNATTENDED_DENY_HINT = (
    "[unattended mode] 本项目已登记为无人值守模式：审批请求不发起、不等待"
    "（本条未经 120s 超时，也不是审核人拒绝）。请改走可审计的替代方案：说明"
    "原因、替代步骤与产出，走 review/汇报；若该操作是平台不可绕过的硬门，请把"
    "目标拆小或调整参数后再发起一次不同的请求。"
)
