...
- 键 = (agent_id, ``failure_signature.signature_of(error_text)``)（空白归一
  ...
  ⚠ **0-4（09-16）起："按因计数"的口径里不再包含平台自产标识**：
  `signature_of` 现在会把 agent 短号（`A075`）与 8 位十六进制 task id
  归一成占位符 ⇒ **同一门禁、不同 task id** 的连拒从此计为**同一个因**
  （改动前各算各的）。方向是**有意的**：本模块存在的理由正是抓"同一堵墙
  被同一 agent 反复撞"，而 task id 每次调用都不同、把它算进身份会让计数
  永远停在 1（正是它要治的病）。不同 gate / 不同文件路径仍然分开计。
