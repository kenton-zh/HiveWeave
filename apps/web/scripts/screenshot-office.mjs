 ...
 */
const emit = () => {
  const payload = { base: BASE, project: PROJECT, projectSwitched, geom, problems, diag, clickLog, windowOpened, mockHits: page.__mockHits ? [...new Set(page.__mockHits)] : [] };
  try {
...
try {
      ...
      .evaluate(() => {
        ...
        try {
          for (const [id, actor] of s.actorMap) {
            const cont = actor?.container;
            if (!cont || cont.visible === false) continue;
  ...
  if (!fatal && canvasBox) {
    ...
    for (const cand of candidates.slice(0, 12)) {
      ...
      if (titles.length > baseline) { windowOpened = titles; await shot("04-window.png"); break; }
    }
